...
long allOddBits(long x) {
    long oneZero = 1<<1;
    long oneZeroOneZero = oneZero | (oneZero << 2);
    long mask = oneZeroOneZero | (oneZeroOneZero << 4);
    mask = mask | (mask << 8);
    mask = mask | (mask << 16);
    mask = mask | (mask << 32);
    return !((x & mask) ^ mask);
 ...
 *  Round toward zero
 *   Examples: dividePowr2(15L,1L) = 7L, dividePower2(-33L,4L) = -2L
 *   Legal ops: ! ~ & ^ | + << >>
 *   Max ops: 15
 *   Rating: 2
 */
long dividePower2(long x, long n) {
    long negativeNumber = x>>63;
    long res = ((negativeNumber&1)<<n)+negativeNumber;
    return (x + res) >> n;
}
// 3
/*
...
long remainderPower2(long x, long n) {
    long isNegative = x >> 63;
    long adjustment = isNegative & 1;
    long quotient = (x + (adjustment << n) + isNegative) >> n;
    return x + (~ (quotient << n) + 1);
}
/*
...
long allAsciiDigits(long x) {
    long pattern0 = 0x10;
    pattern0 = pattern0 | (pattern0 << 8);
    pattern0 = pattern0 | (pattern0 << 16);
    pattern0 = pattern0 | (pattern0 << 32);
    long pattern1 = pattern0 | (pattern0 << 1) | (pattern0 << 2) | (pattern0 << 3);
    long result = 0;
    long pattern2 = pattern0 | (pattern0 << 1);

    result |= (x & pattern1) ^ pattern2;

    long pattern3 = pattern0 >> 1;
    long temp = pattern3 & x;
    long pattern4 = (pattern3 >> 1) | (pattern3 >> 2);
    result |= ((temp >> 1) | (temp >> 2)) & (x & pattern4);
...
long trueThreeFourths(long x) {
    long isNegative = x >> 63;
    long lastTwoBits = x & 3;

    x >>= 2;
    x += (x << 1);
    lastTwoBits += (lastTwoBits << 1) + (isNegative & 3);
    x += lastTwoBits >> 2;
